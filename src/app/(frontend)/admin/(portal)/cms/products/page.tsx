...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const products = await payload.find({ collection: "products", limit: 100, overrideAccess: false, sort, user, where: and.length ? { and } : undefined });
    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      {products.docs.length ? (
        <div className="mt-6 overflow-hidden rounded-2xl border bg-card">
          ...
          {products.docs.map((product) => <div className="grid gap-2 border-b px-4 py-4 last:border-b-0 md:grid-cols-[minmax(0,2fr)_1fr_1fr_1fr_auto_auto] md:items-center md:gap-4" key={product.id}><div className="min-w-0"><p className="truncate text-sm font-medium">{product.name}</p><p className="truncate text-xs text-muted-foreground">{product.brand} · Updated {new Intl.DateTimeFormat("en-AU", { dateStyle: "medium" }).format(new Date(product.updatedAt))}</p></div><p className="text-sm capitalize text-muted-foreground">{product.type}</p><p className="text-sm text-muted-foreground">{product.category}</p><p className="text-sm tabular-nums">{formatPrice(product.price)}</p><p className="text-xs text-muted-foreground">{product.staffPick ? "Yes" : "No"}</p><Button asChild size="sm" variant="ghost"><Link href={`/admin/cms/products/${product.id}`}>Edit</Link></Button></div>)}
        </div>
